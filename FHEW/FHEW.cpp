...
unsigned long fwrite_ek(const EvalKey& EK, char* f) {
	unsigned long fp = 0;
	for (int i = 0; i < n; ++i)
		for (int j = 1; j < BS_base; ++j)
			for (int k = 0; k < BS_exp; ++k) {
				assert(memcpy(f+fp, EK.BSkey[i][j][k], sizeof(ct_FFT)));
				fp+=sizeof(ct_FFT);
			}
	  for (int i = 0; i < N; ++i)
		  for (int j = 0; j < KS_base; ++j)
			  for (int k = 0; k < KS_exp; ++k) {
				  assert(memcpy(f+fp,EK.KSkey[i][j][k], sizeof(LWE::CipherTextQ)));
				  fp+=sizeof(LWE::CipherTextQ);
			  }
	return fp;
}

EvalKey* fread_ek(char* f) {
	...
	for (int i = 0; i < n; ++i)
		for (int j = 1; j < BS_base; ++j)
			for (int k = 0; k < BS_exp; ++k) {
				EK->BSkey[i][j][k] = (ct_FFT*) fftw_malloc(sizeof(ct_FFT));
				assert(memcpy(EK->BSkey[i][j][k], f+fp, sizeof(ct_FFT)));
				fp+=sizeof(ct_FFT);
			}
	// Read switching key
	cerr << "BSKey Read. \n";
	for (int i = 0; i < N; ++i)
		for (int j = 0; j < KS_base; ++j)
			for (int k = 0; k < KS_exp; ++k) {
				...
				assert(memcpy(EK->KSkey[i][j][k], f+fp, sizeof(LWE::CipherTextQ)));
				fp+=sizeof(LWE::CipherTextQ);
			}
	return EK;
}
